...
def number_coalitions_weighting_x(quota, weights):
    ...
    C[Wsum] = 1
    maxQuWsumcum = (
        np.maximum(quota, Wsum - np.cumsum(W, dtype=np.int64)) + W
    )  # maxQuWsumcum = max{quota+W_i,Wsum-Wacc incl i}
    for i in range(n):
...
def number_coalitions_weighting_x_having_size_s(quota, weights):
    ...
    for i in range(n):
        C[maxQuWsumcum[i] - W[i] : Wsum + 1 - W[i], :n] += C[
            maxQuWsumcum[i] : Wsum + 1, 1 : n + 1
        ]
...
def compute_ssi(quota, weights, minimalWinningCoalitionSize=1):
    ...
    for i in range(n):
        w_i = weights[i]
        Cwith_i = number_coalitions_weighting_x_having_size_s_including_i(
            quota, weights, C, i
        )

        for s in range(n):
            t = Cwith_i[quota : quota + w_i, s + 1].sum(axis=0)
            # lowest = gcd(t, )
...
def compute_csi(quota, weights, minimalWinningCoalitionSize=1):
    ...
    CSIfactors = [0] + [
        [
            fac(n - t) / fac(n) * fac(s + t - 1) / fac(s) / 2 ** (s + t - 1)
            for s in range(n - t + 1)
        ]
        for t in range(1, n + 1)
    ]  # [0] is for being able to call t=1,... later
    CSI = [0] * n
    for i in range(n):
        w_i = weights[i]
        Cwith_i = number_coalitions_weighting_x_having_size_s_including_i(
        ...
        if minimalWinningCoalitionSize > 1:
            for s in range(n - minimalWinningCoalitionSize + 1):
                CSI[i] += CSIfactors[minimalWinningCoalitionSize][s] * Cwith_i[
                    quota + w_i : Wsum + 1, minimalWinningCoalitionSize
                ].sum(axis=0)
    return CSI
